    ...
    try {
        ...
        stage("Test") {
            ...
            withEnv(["RESOLWE_POSTGRESQL_USER=postgres",
                ...
                // documentation, linters and packaging environments are run first so that if any
                // of them fails, developer will get the feedback right away (rather than having to
                // wait for all ordinary tests to run)
                sh "tox -e docs"
