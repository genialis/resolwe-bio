...
class ImportFastaNucleotide(Process):
        ...
        try:
            with dnaio.open(infile, fileformat='fasta') as fasta_file:
                if not any(fasta_file):
                    self.error(f'The uploaded .FASTA file {infile} contains no sequence data.')
                else:
    ...
    def run(self, inputs, outputs):
        """Run the analysis."""
        fasta_path = inputs.src.import_file(imported_format='extracted', progress_to=0.2)

        supported_extensions = ('.fa', '.fasta', '.faa', '.fna', '.ffn', '.frn')
        if not fasta_path.endswith(supported_extensions):
            self.error(f'The imported file has unsupported file name extension. '
        ...
        Cmd['picard-tools']['CreateSequenceDictionary', f'R={output_fasta.name}', f'O={fasta_dict}']()
        self.progress = 0.8
