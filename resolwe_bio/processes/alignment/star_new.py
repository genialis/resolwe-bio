...
class AlignmentStar(Process):
    """Align reads with STAR aligner (STAR version 2.7.10b).
    ...
    class Input:
        ...
        gene_counts = BooleanField(
            ...
            description="With this option set to True STAR will count number reads per gene "
    ...
    def run(self, inputs, outputs):
        ...
        if inputs.t_coordinates.quant_mode:
            ...
            if not gene_segments.is_file() and not inputs.annotation:
                self.error(
                    "Counting number of reads per gene requires genome annotation file."
                )
