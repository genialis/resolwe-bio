...
class BsConversionRate(Process):
    ...
    class Input:
        ...
        count_all = BooleanField(label='Count all cytosines including CpGs', default=True)
        read_length = IntegerField(label='Average read length', default=150)
        max_mismatch = FloatField(label='Maximum fraction of mismatches', required=False)
    ...
    def run(self, inputs, outputs):
        ...
        if not skip_process:
            ...
            args = [
                '-chrom', inputs.sequence.fasta.path,
                '-output', report_file,
            ]
            if inputs.count_all:
                args.append('-all')
