...
class BBDukSingle(Process):
    """Run BBDuk on single-end reads (beta).
    ...
    def run(self, inputs, outputs):
        ...
        for lane in range(num_of_lanes):
            reads_basename = Path(inputs.reads.output.fastq[lane].path)
        ...
        fastqgz = [fastq["output_fastq"] for fastq in input_reads]
...
class BBDukPaired(Process):
    """Run BBDuk on paired-end reads (beta).
    ...
    def run(self, inputs, outputs):
        ...
        for lane in range(num_of_lanes):
            ...
            reads_mate2_basename = Path(inputs.reads.output.fastq2[lane].path)
            shutil.copy(reads_mate2_basename, Path.cwd())

            new_path_mate2 = Path(reads_mate2_basename.name.replace(" ", "_"))
            Path(reads_mate2_basename.name).rename(new_path_mate2)
            assert new_path_mate2.name.endswith(".fastq.gz")
            name_mate2 = new_path_mate2.name[:-9]
        ...
        fastqgz = [fastq["output_fastq"] for fastq in input_reads]
        fastqgz2 = [fastq["output_fastq2"] for fastq in input_reads]
        fastqc_inputs = fastqgz + ["--extract", f"--outdir={str(output_path)}"]
