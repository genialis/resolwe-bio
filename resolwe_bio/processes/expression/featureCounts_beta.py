...
def get_gene_counts(infile, outfile, sample_name):
    ...
    if len(filter_col) > 1:
        exp["sum_count"] = exp[filter_col].sum(axis=1)
        exp = exp.astype({"sum_count": int})

        return exp[["sum_count"]].to_csv(
            outfile,
        ...
        exp = exp.astype({sample_name: int})
        return exp[[sample_name]].to_csv(
            outfile,
            index_label="FEATURE_ID",
            header=["EXPRESSION"],
            sep="\t",
...
class FeatureCounts(ProcessBio):
    """Quantify sequencing reads aligned to genomic features (beta).
    ...
    slug = "feature_counts-beta"
    name = "featureCounts (beta)"
    requirements = {
    ...
    def run(self, inputs, outputs):
        ...
        if inputs.general.by_read_group:
            ...
            (Cmd["samtools"]["view", "-H"][bam_file] > "header.sam")()
            return_code, _, _ = Cmd["grep"]["@RG"]["header.sam"] & TEE(retcode=None)
            if not return_code:
