...
from resolwe.process import Process, DataField, FileField, Cmd, ListField, StringField, FileHtmlField
...
class ReverseComplementSingle(Process):
    ...
    def run(self, inputs, outputs):
        ...
        _, _, stderr = Cmd['fastqc'][complemented_name, '--extract', '--outdir=./'] & TEE
...
class ReverseComplementPaired(Process):
    ...
    def run(self, inputs, outputs):
        ...
        if inputs.select_mate == 'Mate 1':
            ...
            _, _, stderr = Cmd['fastqc'][complemented_mate1, '--extract', '--outdir=./'] & TEE
            if 'Failed to process' in stderr or 'Skipping' in stderr:
                self.error('Failed while processing with FastQC.')

            _, _, stderr2 = Cmd['fastqc'][original_mate2, '--extract', '--outdir=./'] & TEE
            ...
            (Cmd['seqtk']['seq', '-r', f'{name_mate2}_original.fastq.gz'] > complemented_mate2)()

            _, _, stderr = Cmd['fastqc'][original_mate1, '--extract', '--outdir=./'] & TEE
            if 'Failed to process' in stderr or 'Skipping' in stderr:
                self.error('Failed while processing with FastQC.')

            _, _, stderr2 = Cmd['fastqc'][complemented_mate2, '--extract', '--outdir=./'] & TEE
            if 'Failed to process' in stderr2 or 'Skipping' in stderr2:
                self.error('Failed while processing with FastQC.')
            ...
            complemented_mate1 = f'{name_mate1}_complemented.fastq'
            complemented_mate2 = f'{name_mate2}_complemented.fastq'
            (Cmd['seqtk']['seq', '-r', f'{name_mate1}_original.fastq.gz'] > complemented_mate1)()
