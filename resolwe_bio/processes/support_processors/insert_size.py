...
class InsertSizeMetrics(Process):
    ...
    def run(self, inputs, outputs):
        ...
        args = [
            ...
            '--INCLUDE_DUPLICATES', inputs.include_duplicates,
        ...
        if 0 <= inputs.minimum_fraction <= 0.5:
            ...
            self.warning('Minimum fraction of reads should be between 0 and 0.5. '
                         'Setting minimum fraction of reads to 0.')
            args.extend(['--MINIMUM_PCT', 0])

        return_code, _, _ = Cmd['gatk']['CollectInsertSizeMetrics'][args] & TEE(retcode=None)
        if return_code:
