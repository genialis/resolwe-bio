...
class AlignmentSummary(Process):
    ...
    class Input:
        ...
        adapters = DataField('data:seq:nucleotide', label='Adapter sequences', required=False)
    ...
    def run(self, inputs, outputs):
        ...
        args = [
            ...
            '--VALIDATION_STRINGENCY', inputs.validation_stringency,
            '--MAX_INSERT_SIZE', inputs.insert_size,
            '--ASSUME_SORTED', inputs.assume_sorted,
            '--EXPECTED_PAIR_ORIENTATIONS', inputs.pair_orientation,
            '--IS_BISULFITE_SEQUENCED', inputs.bisulfite,
        ]
        ...
        return_code, _, _ = Cmd['gatk']['CollectAlignmentSummaryMetrics'][args] & TEE(retcode=None)
        if return_code:
