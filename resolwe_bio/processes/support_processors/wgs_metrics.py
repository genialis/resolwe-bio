...
class InsertSizeMetrics(Process):
    ...
    def run(self, inputs, outputs):
        ...
        args = [
            '--INPUT', inputs.bam.bam.path,
            '--OUTPUT', metrics_file,
            '--REFERENCE_SEQUENCE', inputs.genome.fasta.path,
            '--READ_LENGTH', inputs.read_length,
            '--INCLUDE_BQ_HISTOGRAM', inputs.create_histogram,
            '--MINIMUM_MAPPING_QUALITY', inputs.options.min_map_quality,
            '--MINIMUM_BASE_QUALITY', inputs.options.min_quality,
            '--COVERAGE_CAP', inputs.options.coverage_cap,
            '--LOCUS_ACCUMULATION_CAP', inputs.options.accumulation_cap,
            '--COUNT_UNPAIRED', inputs.options.count_unpaired,
            '--SAMPLE_SIZE', inputs.options.sample_size,
            '--VALIDATION_STRINGENCY', inputs.options.validation_stringency,
        ]
