...
class ReportProcessorTestCase(BioProcessTestCase):
    ...
    def test_amplicon_report(self):
        with self.preparation_stage():
            coverage, pcr_metrics, master_file, annot_vars = self.prepare_report_inputs(
                ...
                summaries=[
                    "56GSID_1k.gatkHC_snpEff_summary.html",
                    "56GSID_1k.gatkHC_snpEff_summary.html",
                ],
                snpeffs=[
                    "56GSID_1k.gatkHC_snpEff_genes.txt",
                    "56GSID_1k.gatkHC_snpEff_genes.txt",
                ],
            )
        ...
        self.assertFiles(
            report,
            "variant_tables",
            ["56GSID.gatkHC.finalvars.txt", "56GSID.lf.finalvars.txt"],
        )
    ...
    def test_multisample_report(self):
        ...
        process.run[
            "program"
        ] += 're-save-file covplot_html "htmlplot/${SAMPLE_SLUG}_coverageplot.html" htmlplot_cp\n'
        process.save()
        ...
        multireport_inputs = {
            "data": [
                ...
                report.id,
                coverage.id,
            ],  # Pretend as there are two distinct reports/samples
            "fields": ["amplicon_cov", "covplot_html"],
        }
        multireport = self.run_process(
            "amplicon-archive-multi-report", multireport_inputs
        )
        ...
        index = next(
            (
                i
                for (i, output) in enumerate(report.process.output_schema)
                if output["name"] == "stats"
            )
        )
