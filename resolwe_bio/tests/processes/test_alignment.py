...
class AlignmentProcessorTestCase(KBBioProcessTestCase):
    ...
    def test_star_beta(self):
        ...
        with self.preparation_stage():
            ...
            single_lanes = self.prepare_reads(
                [
                    ...
                    str("hs sim_reads_single.fastq.gz"),
                ]
            )
            paired_reads = self.prepare_paired_reads(
                mate1=[
            ...
            paired_lanes = self.prepare_paired_reads(
                mate1=[
                    ...
                    str("hs sim_reads1.fastq.gz"),
                ],
                mate2=[
                    str(
                        input_folder / "hs_paired_R2 workflow_bbduk_star_htseq.fastq.gz"
                    ),
                    str("hs sim_reads2.fastq.gz"),
                ],
            )
            annotation = self.prepare_annotation(
                fn=str(input_folder / "hs annotation.gtf.gz"),
        ...
        self.assertFile(
            aligned_reads,
            "stats",
            str(output_folder / "single_lanes_stats.txt"),
            file_filter=filter_star_report,
        )
        ...
        self.assertFile(
            aligned_reads,
            "stats",
            str(output_folder / "paired_lanes_stats.txt"),
            file_filter=filter_star_report,
        )
