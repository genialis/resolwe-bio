...
class MappingTestCase(APITestCase, ElasticSearchTestCase):
    ...
    def test_admin(self):
        ...
        response = self.client.get(
            reverse("resolwebio-api:mapping-list"), format="json"
        )
        ...
        self.assertEqual(len(response.data), len(self.mappings))
        for data, mapping in zip(
            sorted(response.data, key=lambda x: x["id"]), self.mappings
        ):
        ...
        self.assertEqual(
            Mapping.objects.get(source_db="SRC", source_id="MYSRCID").target_id,
            "MYTGTID",
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Mapping.objects.get(source_db="SRC", source_id="MYSRCID").relation_type,
