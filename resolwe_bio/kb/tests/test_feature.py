...
class FeatureTestCase(APITestCase, ElasticSearchTestCase):
    ...
    def test_feature_admin(self):
        ...
        response = self.client.get(
            reverse("resolwebio-api:feature-list"), format="json"
        )
        ...
        for data, feature in zip(
            sorted(response.data, key=lambda x: x["id"]), self.features
        ):
        ...
        self.assertEqual(
            Feature.objects.get(source="NCBI", feature_id="TEST").full_name, "Test test"
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Feature.objects.get(source="NCBI", feature_id="TEST").full_name,
            "Modified test",
        )
