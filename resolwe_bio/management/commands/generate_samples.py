...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-a', '--n-annotated', type=int, default=15,
                            help="Number of annotated samples to generate (default: %(default)s)")
        parser.add_argument('-u', '--n-unannotated', type=int, default=5,
                            help="Number of unannotated samples to generate (default: %(default)s)")
    ...
    def generate_expressions(self, gene_ids, path):
    ...
    def generate_reads_descriptor(self):
    ...
    def generate_sample_desciptor(self, organism_name):
    ...
    def create_data(self, reads_name='seq_reads', annotated=False, rseed=None):
        ...
        if annotated:
            ...
            sample.save()
            print "Created annotated sample: {} (id={})".format(sample.name, sample.id)
        else:
            print "Created unannotated sample: {} (id={})".format(sample.name, sample.id)

    def handle(self, *args, **options):
        if options['rseed']:
            random.seed(42)
        for i in range(options['n_annotated']):
            self.create_data(annotated=True)
        for i in range(options['n_unannotated']):
