...
with open(sys.argv[1]) as f:
    ...
    for line in f:
        ...
        if 'overall alignment rate' in line:
            ...
            stats.append((
                iteration, processed, not_aligned,
                unique, multiple, discord, mapped))
            processed, not_aligned, unique, multiple, discord, mapped = (
                0, 0, 0, 0, 0, 0)

with open('stats.tab', 'w') as f:
    f.write("Alignment\tPairs processed\tPairs aligned 0 times\t"
    ...
    t_mapped = round(
        ((t_unique + t_multiple + t_discord) / stats[0][1]) * 100, 1)

    f.write("\t".join(map(str, (
        "Total", stats[0][1], stats[-1][2],
        t_unique, t_multiple, t_discord, t_mapped))) + "\n")
