...
def parse_arguments():
    ...
    parser.add_argument('--exp_types', help='List of expression types (TPM, SMP, FPKM...)', nargs='+')
    parser.add_argument('--spikeins_mix', help="Used spike-ins mix.", choices=ERCC_MIXES.keys())
...
def get_expected(spikeins_mix, log2=False):
    ...
    """
    ercc_data = pd.read_csv(ERCC_CONC_FILE, delimiter='\t', dtype=str, index_col="ERCC ID")
...
def plot_histogram(histogram, all_spikeins, nonzero_spikeins, spikein_type):
    ...
    histogram.bar(nonzero_spikeins.index, nonzero_spikeins, color='green', alpha=0.5, width=0.4)
...
def plot_scatter(scatter, zero, nonzero, exp_type):
    ...
    slope, intercept, r_value, _, _ = stats.linregress(nonzero.iloc[:, 0], nonzero.iloc[:, 1])
    if not zero.empty:
        scatter.scatter(zero.iloc[:, 0], intercept + zero.iloc[:, 0] * slope, alpha=0.5, color='grey')
    ...
    scatter.text(
        ...
        s='\n'.join([
            ...
            "{} / {} transcripts not detected".format(len(zero), len(zero) + len(nonzero)),
        ]),
...
def plot_histogram_scatter(expected, zero, nonzero, spikein_type, sample_name, exp_type):
